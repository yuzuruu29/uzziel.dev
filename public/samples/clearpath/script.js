/* ClearPath Lawn Care: navigation, FAQ, quote validation, and image reveal. */
(function () {
  'use strict';
  ...
  function updateServiceValidity() {
    if (!serviceCheckboxes.length) return true;

    var hasService = Array.prototype.some.call(serviceCheckboxes, function (checkbox) {
      return checkbox.checked;
    });

    serviceCheckboxes[0].setCustomValidity(
      hasService ? '' : 'Select at least one service.'
    );
    return hasService;
  }

  Array.prototype.forEach.call(serviceCheckboxes, function (checkbox) {
    checkbox.addEventListener('change', updateServiceValidity);
  });

  if (form && thankYou) {
    form.addEventListener('submit', function (e) {
      e.preventDefault();
      updateServiceValidity();
